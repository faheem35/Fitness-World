...
// ✅ Updated: Function to check if current time is within 8–10 AM or 7–9 PM
function isWithinAllowedTime() {
  const now = new Date()
  const hours = now.getHours()
  return (hours >= 8 && hours < 10) || (hours >= 19 && hours < 21)
}

// ✅ Updated: Get next session time based on the new schedule
function getNextSessionTime() {
  const now = new Date()
  const hours = now.getHours()

  if (hours < 8) return "morning session at 8:00 AM"
  if (hours >= 10 && hours < 19) return "evening session at 7:00 PM"

  const tomorrow = new Date()
  tomorrow.setDate(tomorrow.getDate() + 1)
  return "morning session tomorrow at 8:00 AM"
}
                ...
                {!joined ? (
                  <>
                    {showTimeAlert && (
                      <Alert variant="danger" style={styles.timeAlert}>
                        <i className="fa-solid fa-circle-exclamation me-2"></i>
                        Sessions are only available during morning (8–10 AM) and evening (7–9 PM)
                      </Alert>
                    )}
                    <div style={styles.sessionInfo}>
                      <div className="d-flex justify-content-between align-items-center flex-wrap mb-3">
                      ...
                      <div style={styles.timeSlots}>
                        <div style={styles.timeSlot}><i className="fa-solid fa-sun me-2"></i> Morning: 8:00 AM – 10:00 AM</div>
                        <div style={styles.timeSlot}><i className="fa-solid fa-moon me-2"></i> Evening: 7:00 PM – 9:00 PM</div>
                      </div>
